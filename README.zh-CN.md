...
Session binding 始终把工作区工具限制在当前 run 的不可变目录快照内。`agent.permissions.workspaceMutation` 默认是 `plan`；改为 `allow` 会关闭工作区修改的 Plan admission 门禁，包括声明 `workspaceMode=write` 的 `bash` 调用。在 macOS 上，`bash` 是独立授权的项目调试能力：要求绑定工作区，以关闭 stdin 的非交互 `/bin/bash` 执行有界命令，并继续硬拒绝破坏性命令。`workspaceMode=read` 只允许写入 Kernel 持有的 `$TMPDIR`；`workspaceMode=write` 在修改工作区文件前还必须被已确认 Plan 中的精确操作覆盖。工作目录固定为绑定工作区根目录，进入子目录需在命令中显式使用 `cd`。PATH 由 Host PATH 与当前存在的标准开发工具目录组成；`HOME` 可读但不可写。Kernel 在每条可达的调用退出路径清理私有临时目录。退出码为零生成 completed ToolRecord；非零退出或超时生成 failed ToolRecord，同时保留有界 stdout、stderr、退出码和超时事实。在实现其他平台的工作区沙箱前，工具目录只在 macOS 把 `bash` 标成 callable，其余平台标成 blocked。网络读取默认 `allow`，可以访问 Host 任意可达的 HTTP(S) 地址；外部副作用工具仍默认 `ask`。`agent.web.search.endpointTemplate` 留空时，`web.search` 使用内建 Bing RSS；显式模板继续按现有 JSON 搜索结果合同覆盖内建后端。两种设置都不会放宽工作区路径解析或工作区外副作用确认。`agent.permissions.engineeringDecisions` 独立控制实质工程路线不明确时通过 `interaction.request` 询问，还是委托 Agent 依据当前代码事实自行裁决。公开 GitHub 仓库/Issue 搜索和 contents 读取无需凭据；GitHub REST code search 按官方要求使用 daemon 启动时捕获的 `DEEPCODE_GITHUB_TOKEN`。
