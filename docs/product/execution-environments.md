...
Native Windows uses an invocation-specific Low Privilege AppContainer (LPAC). The support check launches a real child and verifies its AppContainer token and `WIN://NOALLAPPPKG` opt-out. Store PowerShell aliases resolve to the registered package executable, so automatic selection can use PowerShell 7 inside the sandbox. PowerShell receives the Windows runtime capabilities it needs for initialization; Git for Windows receives read access to its recognized installation directory, including helpers and libraries.

Known limitation: launching another Store PowerShell 7.6.6 process from inside an LPAC invocation can fail with Windows error 87. This is distinct from using PowerShell 7 as the main Shell. The cause remains undetermined; preserve the startup error rather than treating it as a command syntax problem or changing execution scope automatically.

The Windows backend applies the shared workspace file scope before resuming the child. Ancestors receive only traversal and attribute access, without directory listing or inheritance; admitted files and directories receive their scoped permissions. ACL checks use the actual child token, including package claims. Temporary invocation ACEs and the AppContainer profile are removed after execution, and child processes remain owned by a Job Object. A failed support check reports the original error; it does not switch to Host execution.
...
Shell approval and execution scope are separate settings. Ask waits for the user; Review invokes an independent context with no tools; Allow uses the permitted environment without further Shell review. A delegated reviewer decides only the current operation; each subsequent request is reviewed separately. Reusable grants require an explicit user decision. The selected review model or its configuration error is captured for the run. An unavailable reviewer leaves ordinary conversation usable and reports its error with a request for human approval when review is needed. Changing relevant permission settings or revoking a grant takes effect before the next execution.
