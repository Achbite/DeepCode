...
fn register_pending_plans_from_projection(
    gui: &mut GuiState,
    session_id: &str,
    events: &[Value],
) {
    for event in events {
        ...
        if kind == "plan_card" {
            if let Some(plan) = pending_plan_from_plan_card_event(session_id, event) {
                insert_pending_plan_aliases(gui, plan);
            }
            continue;
        }
        if kind == "plan_review" {
            ...
            let Some(plan_id) = payload.get("planId").and_then(Value::as_str) else {
                continue;
            };
            if let Some(plan) = find_pending_plan_by_alias(gui, plan_id) {
                remove_pending_plan_aliases(gui, &plan);
            }
        }
    }
}

pub(crate) fn pending_plan_from_plan_card_event(
    fallback_session_id: &str,
    event: &Value,
) -> Option<PendingAgentPlan> {
    let payload = event.get("payload")?;
    let action_bundle = payload.get("actionBundle")?.clone();
    let plan_id = payload
        .get("planId")
        .and_then(Value::as_str)
        .or_else(|| action_bundle.get("id").and_then(Value::as_str))
        .or_else(|| payload.get("proposalId").and_then(Value::as_str))?
        .to_string();
    let run_id = payload.get("runId").and_then(Value::as_str)?.to_string();
    let session_id = event
        .get("sessionId")
        .and_then(Value::as_str)
        .unwrap_or(fallback_session_id)
        .to_string();
    let user_plan = payload
        .get("content")
        .and_then(Value::as_str)
        .or_else(|| payload.get("summary").and_then(Value::as_str))
        .unwrap_or("Agent plan")
        .to_string();
    let code_blocks = payload
        .get("codeBlocks")
        .and_then(Value::as_array)
        .cloned()
        .unwrap_or_default();
    Some(PendingAgentPlan {
        session_id,
        run_id,
        plan_id,
        proposal_id: payload
        ...
        action_bundle,
        code_blocks,
        expected_validation: payload
            .get("expectedValidation")
            .and_then(Value::as_str)
            .unwrap_or("")
            .to_string(),
        review_guide: payload
            .get("reviewGuide")
            .and_then(Value::as_str)
            .unwrap_or("")
            .to_string(),
        plan_review_report: payload.get("planReviewReport").cloned().filter(|value| !value.is_null()),
        created_at: event
            .get("ts")
            .and_then(Value::as_str)
            .map(ToOwned::to_owned)
...
pub(crate) fn insert_pending_plan_aliases(gui: &mut GuiState, plan: PendingAgentPlan) {
    for alias in pending_plan_aliases(&plan) {
        gui.pending_plans.insert(alias, plan.clone());
    }
}

pub(crate) fn remove_pending_plan_aliases(gui: &mut GuiState, plan: &PendingAgentPlan) {
    for alias in pending_plan_aliases(plan) {
        gui.pending_plans.remove(&alias);
    }
}

pub(crate) fn find_pending_plan_by_alias(gui: &GuiState, alias: &str) -> Option<PendingAgentPlan> {
    gui.pending_plans.get(alias).cloned()
}

pub(crate) fn pending_plan_aliases(plan: &PendingAgentPlan) -> BTreeSet<String> {
    let mut aliases = BTreeSet::new();
    if !plan.plan_id.trim().is_empty() {
        aliases.insert(plan.plan_id.clone());
    }
    if let Some(proposal_id) = plan.proposal_id.as_ref().filter(|value| !value.trim().is_empty()) {
        aliases.insert(proposal_id.clone());
    }
    if let Some(action_id) = plan
        .action_bundle
        .get("id")
        .and_then(Value::as_str)
    ...
    if let Some(report_plan_id) = plan
        .plan_review_report
        .as_ref()
        .and_then(|report| report.get("planId"))
        .and_then(Value::as_str)
        .filter(|value| !value.trim().is_empty())
    {
        aliases.insert(report_plan_id.to_string());
    }
    aliases
}
