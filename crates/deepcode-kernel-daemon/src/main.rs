...
fn kernel_ledger_path() -> Option<PathBuf> {
...
async fn skill_mount_scan(Json(body): Json<SkillMountScanRequest>) -> Json<ApiResponse> {
...
fn scan_skill_manifest(mount_root: &FsPath, dir: &FsPath, path: &FsPath) -> Result<Value, String> {
...
fn scan_skill_markdown(mount_root: &FsPath, dir: &FsPath, path: &FsPath) -> Result<Value, String> {
    let raw = fs::read_to_string(path)
        .map_err(|error| format!("read {} failed: {error}", path.display()))?;
    let title = raw
        .lines()
...
fn string_vec_field(value: &Value, field: &str) -> Vec<String> {
    value
        .get(field)
        .and_then(Value::as_array)
        .map(|items| {
...
fn relative_path_text(root: &FsPath, path: &FsPath) -> String {
...
async fn api_not_implemented(method: Method, Path(path): Path<String>) -> Json<ApiResponse> {
    ApiResponse::error(
        "api_not_implemented",
        format!("Route {method}:/api/{path} is not implemented by the Rust Kernel Daemon yet."),
    )
}
