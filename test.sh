...
search "probeAuthoritativeDocs|AuthoritativeDocExcerpt|lineStart|lineEnd|heading|excerptHash|docExcerptHash" userspace/session-core/src/prompt/docProbe.ts >/dev/null
search "SyntheticDialoguePacket|createSyntheticDialoguePacket" userspace/session-core/src/prompt/dialogue.ts >/dev/null
search "ContextSnapshot|currentUserOverlayHash|rulerHash|builtinSystemPromptHash|protocolContractHash|docExcerptHash|resourcePacketHashes|auditHash" userspace/session-core/src/prompt/contextSnapshot.ts >/dev/null
search "PromptEnvelopeBuilderInput|PromptSystemLayer|buildPromptEnvelope|protocolContract|builtinSystemPrompt|rulerContext|currentUserOverlay|authoritativeDocExcerpts|memoryHints|resourceContext|auditOnlyContext|auditOnlyLayerNames" userspace/session-core/src/prompt >/dev/null
search "canonicalizePrompt|canonicalizeToolSchema|stablePrefixHash|dynamicSuffixHash|auditHash|cacheHash|LocalL2Cache|SingleflightDeduper|applyProviderCacheStrategy|deepseek-openai|anthropic-native|ollama|providerTelemetryFromUsage" userspace/session-core/src/cache >/dev/null
! search "Prompt Settings|System Prompt Editor|User Prompt Override|PromptProfiles|Prompt Profiles|PromptSettingsPlaceholder|prompt\\.profiles|prompt\\.defaultProfileId|prompt-profile" userspace config crates README.md \
