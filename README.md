...
| Xcode CLT | macOS 原生打包需要 | 生成 Darwin arm64 Tauri `.app` 与本机二进制 |
...
open ./bin/macos-arm64/DeepCode-TUI.command
```

`make package-macos` 会调用 `scripts/package-macos.sh`，在 macOS arm64 宿主机上生成 Darwin 原生发行目录。该步骤是 macOS `.app` 的显式 host-side 例外：常规开发、Linux/Windows 构建和 Docker 回归仍优先在 `deepcode-dev` 容器中进行；macOS 原生 GUI/TUI 封装默认复用已有 GUI dist，并在最后打包阶段使用宿主机 Rust / Cargo / Xcode Command Line Tools。只有设置 `DEEPCODE_MACOS_BUILD_GUI_ON_HOST=1` 时，脚本才会尝试宿主机 Node / pnpm GUI 构建。

macOS 发行目录固定为：

```text
bin/macos-arm64/DeepCode.app
bin/macos-arm64/deepcode-kernel
bin/macos-arm64/deepcode-cli
bin/macos-arm64/deepcode-tui
bin/macos-arm64/DeepCode-TUI.command
bin/macos-arm64/web/
bin/macos-arm64/README.txt
```

`DeepCode.app` 会直接渲染打包内置的完整 React 工作台，并在后台启动 `.app/Contents/MacOS/deepcode-kernel`。`DeepCode-TUI.command` 是可双击的 Terminal 启动器，会选择空闲本地端口、启动同目录 Kernel、等待 `/api/health`，再进入真实 Ratatui/Crossterm TUI。当前 macOS 包是本机可运行包，不包含 DMG、Developer ID 签名或公证；脚本会尽量做本地 ad-hoc signing。
...
- macOS：`$XDG_CONFIG_HOME/deepcode/config` 或 `~/.config/deepcode/config`
...
- macOS Darwin 原生包通过 `make package-macos` 单独生成与验收
...
- **打包与预览**
  ...
  - `make package-macos` 输出 Darwin 原生 `bin/macos-arm64`，包含 `DeepCode.app` 与 `DeepCode-TUI.command`。
  ...
  - macOS `bin/macos-arm64/DeepCode.app` 是本机 GUI thin shell，会直接渲染内置 React 工作台并在后台启动 `.app/Contents/MacOS/deepcode-kernel`。
  - macOS `bin/macos-arm64/DeepCode-TUI.command` 是可双击 Terminal 启动器，会启动同目录 Kernel 并进入真实 TUI。
...
[23] GUI纯会话框参考：https://github.com/XingYu-Zhong/DeepSeek-GUI.git Accessed: 2026-06-08

TUI/CLI参考感谢PI与codex的开源实现，上文以引用具体仓库不再重复引用
