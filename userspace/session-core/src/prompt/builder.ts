  ...
  const layers = ([
    ...
    },
    {
      ...
      content: [
        ...
        'Use kind="actionBundle" only during Complete stage after Session provides acceptedTaskPlan/accepted plan context, or when the user explicitly asks for a tiny single-step side effect and no intermediate planning is needed. Put userPlanMarkdown, codeBlocks, and actionBundle as top-level fields on that JSON object; do not wrap them inside a payload object. You may include validation notes in actionBundle.validationExpectations[] and review obligations in actionBundle.reviewExpectations[]; Session derives routine defaults when they are omitted.',
        ...
        'Action entries must use actionId, toolId, args, and description. Do not output capability, permissionLabels, accessScopes, or resourceScope; Kernel derives capability, risk, permissions, readSet, writeSet, and conflictKeys from toolId and typed args.',
        ...
        'If the full implementation exceeds the payload budget or needs fresh evidence, represent the full checklist in taskPlan first. During Complete stage, include only current-task notes in non-executable actionBundle.continuationExpectations.',
    ...
    },
    {
      ...
      content: [
        ...
        'actionBundle proposal top-level fields: userPlanMarkdown, codeBlocks, actionBundle. The nested actionBundle object must include version/id/goal/actions; goal is a short batch objective, not a permission grant or execution fact. validationExpectations[] and reviewExpectations[] are optional provider-facing validation/review note carriers; Session derives routine defaults when they are omitted. actionBundle is for Complete stage after acceptedTaskPlan context unless the user explicitly asked for a tiny single-step side effect.',
        ...actionBundleProtocolShapeLines(),
        ...
        'If validation cannot run because a required capability such as process.exec is not approved, either declare reviewable evidence or omit validationExpectations and let Session derive a routine review note.',
