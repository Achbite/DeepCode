  ...
  const layers = ([
    ...
    },
    {
      ...
      content: [
        ...
        'taskPlan top-level field: taskPlan.version/id/title/summary/tasks/risks/reviewCheckpoints. tasks[] is an ordered implementation checklist using taskId/title/target/capability/acceptanceCriteria/failureCriteria. Deprecated graph fields such as executionFlowGraph/hardDependencies/softOrderAfter may be parsed for telemetry, but provider output should not rely on graph scheduling. taskPlan must not include fileOperations, accessScopes, codeBlocks, actionBundle, commandBlocks, patches, source code, or executable tool calls.',
