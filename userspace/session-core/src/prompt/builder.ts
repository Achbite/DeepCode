  ...
  const layers = ([
    ...
    },
    {
      ...
      content: [
        ...
        'File delete actions are first-class actions: use kind="delete", capability="fs.delete", a concrete file targetPath/resourceScope, and permissionLabels ["fs.delete"]. Delete actions must not include codeBlocks, sourceBlockId, embedded content, empty-content writes, or fs.write disguised as deletion.',
        ...
        'Implementation batch budget: at most 4 codeBlocks, at most about 12KB total codeBlock content, and at most about 6KB per codeBlock. Keep actions concrete and reviewable; Kernel will group permission bundles by capability and scope.',
    ...
    },
    {
      ...
      content: [
        ...
        'Agent Protocol v3 implementation plan shape: {"schemaVersion":"deepcode.agent.protocol.v3","kind":"implementationPlan","outputLanguage":"zh-CN","narration":"我先把可审查的任务清单拆出来，确认后再进入逐批编辑。","implementationPlan":{"version":"1","id":"impl-plan-...","title":"...","summary":"...","tasks":[{"taskId":"task-1","title":"...","target":["relative/path-or-directory"],"scope":"what this task intends to do, without source code","dependencies":[],"hardDependencies":[],"softOrderAfter":[],"conflictKeys":["relative/path-or-directory"],"canDraftInParallel":true,"role":"sourceCode","capability":"fs.write","acceptanceCriteria":["reviewable evidence expected after execution"],"failureCriteria":["conditions that should stop or replan"]}],"risks":["..."],"reviewCheckpoints":["user review checkpoint before edits","user review checkpoint after Kernel facts"]}}',
        ...
        'Agent Protocol v3 action bundle shape: {"schemaVersion":"deepcode.agent.protocol.v3","kind":"actionBundle","outputLanguage":"zh-CN","narration":"我已经整理出下一批需要审查的操作，先交给 Kernel 做权限和计划检查。","userPlanMarkdown":"# <localized plan title>\\n\\n## <summary heading>\\n...\\n\\n## <changes heading>\\n- ...\\n\\n## <interfaces or affected surfaces heading>\\n- ...\\n\\n## <validation heading>\\n- ...\\n\\n## <assumptions heading>\\n- ...","codeBlocks":[{"blockId":"...","targetPath":"<workspace-resource>","language":"...","operation":"create","content":"...","permissionLabels":["fs.write"]}],"commandBlocks":[{"commandId":"...","capability":"process.exec","cwd":"<workspace-resource>","argv":["<executable>","<arg>"],"timeoutMs":120000,"envPolicy":"inheritSafe","expectedOutput":"...","permissionLabels":["process.exec"]}],"actionBundle":{"version":"1","id":"...","goal":"...","actions":[{"actionId":"...","description":"...","capability":"fs.write","resourceScope":["<workspace-resource>"],"targetPath":"<workspace-resource>","sourceBlockId":"...","dependsOn":[],"permissionLabels":["fs.write"]},{"actionId":"...","description":"...","kind":"delete","capability":"fs.delete","resourceScope":["<relative-file>"],"targetPath":"<relative-file>","dependsOn":[],"permissionLabels":["fs.delete"]}],"continuationExpectations":[{"id":"next-batch","title":"Continue with the next reviewable implementation batch after user approval","capability":"fs.write","kind":"write","resourceScope":["<workspace-resource>"]}],"validationExpectations":[{"id":"files-written","description":"Kernel records write/delete facts for every planned file and the final review can inspect the changed paths."}],"reviewExpectations":[{"id":"user-review","description":"User reviews this batch scope, generated files, deleted files, and validation evidence before accepting completion."}]},"expectedValidation":"...","reviewGuide":"..."}',
