  ...
  const layers = ([
    ...
    },
    {
      ...
      content: [
        ...
        'taskPlan top-level field: taskPlan.version/id/title/summary/tasks/executionFlowGraph/risks/reviewCheckpoints. tasks list taskId/title/target/capability/dependencies/hardDependencies/softOrderAfter/conflictKeys/canDraftInParallel/acceptanceCriteria/failureCriteria. executionFlowGraph.nodes list nodeId/moduleId/modulePath/taskIds/targets/capabilities/prerequisites/outputs/dependsOn/unlocks/conflictKeys/evidenceNeeds so Session can schedule sub-agent DAG nodes. taskPlan must not include codeBlocks, actionBundle, commandBlocks, patches, source code, or executable tool calls.',
        ...
        'actionBundle proposal top-level fields: userPlanMarkdown, codeBlocks, actionBundle, expectedValidation, and reviewGuide. The nested actionBundle object has version/id/goal/actions/continuationExpectations/validationExpectations/reviewExpectations. actionBundle is for Complete stage after acceptedTaskPlan context unless the user explicitly asked for a tiny single-step side effect.',
        ...actionBundleProtocolShapeLines(),
